...
from models.topology_utils import compute_persistent_diagram
...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            output_dict = classifier(points, target)
            seg_logits = output_dict['logits']
            pred_one_hot = output_dict['pred_one_hot']
            seg_pred_flat = seg_logits.contiguous().view(-1, num_part)
            target_flat = target.view(-1)
            pred_choice = seg_pred_flat.data.max(1)[1]
            correct = pred_choice.eq(target_flat.data).cpu().sum()
            mean_correct.append(correct.item() / (args.batch_size * args.npoint))
            loss = criterion(output_dict, target_flat)
